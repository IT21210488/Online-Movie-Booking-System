...
<?php
    if(isset($_SESSION['staff_id'])){
        ...
        $check_user_query = "SELECT * FROM staffmember WHERE S_ID='$staff_id'";
        ...
        if(mysqli_num_rows($check_user_query_run)>0){
            $_SESSION['rand_otp'] = $fetch_user_details['otp'];
        }else{
            session_destroy();
...
<?php
    if(isset($_POST['verify_otp'])){
        ...
        $db_otp = $_SESSION['rand_otp'];

        // Check expiry date
        $check_exp_query = "SELECT expires FROM staffmember WHERE S_ID='$staff_id'";
        ...
        if($current_time < $exp_date_db){
            if($db_otp == $user_otp){
                $rmv_otp_query = "UPDATE staffmember SET otp=NULL WHERE S_ID='$staff_id'";
                $rmv_otp_query_run = mysqli_query($connect,$rmv_otp_query);

                $rmv_exp_query = "UPDATE staffmember SET expires=NULL WHERE S_ID='$staff_id'";
                $rmv_exp_query_run = mysqli_query($connect,$rmv_exp_query);

                header("location:admin_resetPassword.php");
            }else{
                ...
                <div class="fppage-form-sec">
                    ...
                    <div class="form-other-links-sec">
                        <div class="form-other-links-left">
                            <a href="login.php" class="fppage-other-link-txt">Have password ?</a>
                        </div>
                        <div class="form-other-links-right">
                            <a href="signup.php" class="fppage-other-link-txt">Create a new account ?</a>
                        </div>
                    </div>
                </div>
