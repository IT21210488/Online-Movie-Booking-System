...
<?php 
    if(isset($_POST['request_otp'])){
        ...
        $check_for_user_query = "SELECT * FROM customer WHERE Email='$user_email'";
        ...
        if(mysqli_num_rows($check_for_user_query_run)>0){
            $_SESSION['user_id'] = $fetch_user['C_ID'];
            $record_otp_query = "UPDATE customer SET otp='$rand_otp' WHERE C_ID='$_SESSION[user_id]'";
            $record_otp_query_run = mysqli_query($connect,$record_otp_query);

            $record_expiry_date_query = "UPDATE customer SET expires='$expiry_date' WHERE C_ID='$_SESSION[user_id]'";
            ...
            if (mail($email_recipient, $email_subject, $email_body, $email_headers)) {
                header("location:cust_verifyPasswordOtp.php");
            } else {
